        ...
        <div class="pd20">
            ...
            <div class="input-group listSeaForm mt10 wb100">
                ...
                <select class="form-control" id="department">
                    ...
                    <?php if(is_array($memberlist)): foreach($memberlist as $key=>$v): ?><option value="<?php echo ($v["hospital"]); ?>"><?php echo ($v["hospital"]); ?></option><?php endforeach; endif; ?>
                </select>
            </div>
            <div class="input-group listSeaForm mt10 wb100">
                <span class="input-group-btn">
                    <span class="btn">科分类：</span>
                </span>
                <select class="form-control" id="departmentadd">
        ...
        <div class="pd20">
            <div class="clearfix">
                <div class="l" style="width: 290px;">
                    ...
                    <div class="input-group listSeaForm mt10 wb100">
                        ...
                        <input class="form-control" id="password" type="text" value="" placeholder="请输入密码" >
                    </div>
                    <div class="input-group listSeaForm mt10 wb100">
                        <span class="input-group-btn">
                            <span class="btn">年龄：</span>
            ...
            <div class="input-group listSeaForm mt10 wb100">
                ...
                <select class="form-control" id="b-department_id">
                    <option value="0">请选择科室</option>
                    <?php if(is_array($departmentList)): foreach($departmentList as $key=>$v): ?><option value="<?php echo ($v["did"]); ?>" data_obj="<?php echo ($v["department_name"]); ?>"><?php echo ($v["department_name"]); ?></option><?php endforeach; endif; ?>
                </select>
            </div>
            <div class="input-group listSeaForm mt10 wb100">
                <span class="input-group-btn">
        ...
        $(document).on('click', '.doctorAddBtn', function () {
            ...
            $("#addUser").click(function () {
                ...
                var department = $("#department").val();
                 var ksname = $("#departmentadd").find('option:selected').val();
                ...
                $.post('<?php echo U("Member/addUser");?>',{user_name:user_name,true_name:true_name,
                    age:age,department_id:department_id,department:department,rank:rank,type:type,ksname:ksname},
                    function (data) {
                     if(data.status=='success'){
                         remindBox('添加成功');
                        window.location.reload();
        ...
        $(document).on('click', '.doctorEditBtn', function () {
            ...
            $("#b-save").click(function (data) {
                ...
                var department_id = $("#b-department_id").val();
                 var keshi = $("#b-department_id").find("option:selected").attr('data_obj');
                 // alert(department_id);
                 // alert(keshi);
                var rank = $("#b-rank").val();
                ...
                var age = $("#b-age").val();
                var ksname = $("#departmentedit").find('option:selected').val();
                // alert(ksname);
                ...
                rank:rank,introduction:introduction,strong:strong,type:type,uid:uid,picture:picture,password:password,keshi:keshi,ksname:ksname},function (data) {
                        if(data.status=='success'){
                            remindBox("修改成功");
                            window.location.reload();
                        }else{
                ...
                if (data.status=='success') {
                    if (data.msg.count > 0) {
                        ...
                        $.each(data.msg.list,function (i,val) {
                            ...
                            html +="<td>"+val.ksname+"</td>";
                            html +="<td>"+val.keshi+"</td>";
                            ...
                            if(val.ispic=='1'){
                                html+="<td>已审核</td>";
                            }else if(val.ispic=='0'||val.ispic==''){
                                  html+="<td>未审核</td>";
                            }
                             if(val.pic1!='' && val.pic2!=''&&val.pic3!=''&&val.pic4 !=''){
                                html +="<td><button type='button' uid='"+val.id+"' class='btn btn-success btn-sm EditBtn'>认证申请</button> " ;
                            }else{
                                html +="<td><button type='button' uid='"+val.id+"' class='btn  btn-sm '>认证申请</button> " ;
 
                            }
                              html +="<td><button type='button' uid='"+val.id+"' class='btn btn-success btn-sm doctorEditBtn'>编辑</button> " ;

                            html +="<button type='button' uid='"+val.id+"' class='btn btn-default btn-sm deleteBtn'>删除</button></td>" ;
 ...
 $(document).on('click', '.EditBtn', function () {
  ...
  $(document).on('click', '.edit', function(){
          ...
          var uidlist = $("input[name='shenheid']:checked").val();
            ...
            $.get("<?php echo U('/Member/userList');?>",
                {"uid":uid,
               "uidlist":uidlist},
                function (data) {
