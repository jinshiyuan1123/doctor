...
function like(id) {
	...
	likeID = id
	ajaxPostReq('__ROOT_ADMIN__/php/like.php',{'id':id,'like':likes,'time':get_time()},{"1":'感谢您的支持！','0':"您已支持过了~"},voids)
}
...
function submit_pl(cname) {
	...
	data.time = time
	ajaxPostReq('/admin/php/add.php',data,{'0':'评论失败','1':'评论成功'},reflash)
}
...
function start(mode) {
	ajaxInitReq('__ROOT_ADMIN__/php/handler.php',{'m':mode},render)
}